//! End-to-end: a host inside the virtual network connects to a port the
//! application opened via `slirp::Stack::listen`. We play the role of the
//! virtual client with our own `vtcp::Conn`, inject its segments into the
//! stack as IPv4 packets, and route the stack's emitted packets back into the
//! client. `Listener::accept()` should yield a `slirp::TcpStream`, after which
//! we exchange data in both directions.
#![cfg(feature = "slirp")]
...
const CLIENT_IP: Ipv4Addr = Ipv4Addr::new(10, 0, 0, 5); // peer inside the virtual net
const SERVER_IP: Ipv4Addr = Ipv4Addr::new(10, 0, 0, 1); // the stack's virtual address
